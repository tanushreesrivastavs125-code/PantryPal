...
const EmptyRecipes = ({ onCreateRecipe, isFiltered = false, onResetFilters }) => {
  return (
    <div className="bg-white rounded-2xl border border-[rgba(138,144,112,0.18)] p-8 sm:p-12 text-center flex flex-col items-center justify-center shadow-[0_1px_3px_rgba(39,42,31,0.04)]">
      ...
      <h3 className="text-lg font-bold text-[var(--color-dark)] mb-1">
        {isFiltered ? 'No matching recipes found' : 'No recipes yet'}
      </h3>

      <p className="text-sm text-[var(--color-sage)] max-w-sm mb-6 leading-relaxed">
        {isFiltered
          ? 'Try adjusting your search terms, cuisine selection, or difficulty filters.'
          : 'Build your personal digital cookbook or generate AI recipes tailored to your pantry stock.'}
      </p>
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="primary" size="md" icon={Plus} onClick={onCreateRecipe}>
          Create Recipe
        </Button>
      </div>
    </div>
